...
function PopupWindow({ active, setActive, trainer}) {
	...
	return (
		<div className={active ? "popup popup--active" : "popup"} onClick={() => setActive(false)}>
			<div className='popup_content container' onClick={(e) => e.stopPropagation()}>
				<div className='close_btn row' onClick={() => setActive(false)}>Закрыть</div>
				<div className='row'>
					<div className='col-6 col-sm-3  '>
						<img className='popup_img' src={trainer.image} alt={`trainer ${trainer.id}`}></img>
					</div>
					<div className='trainer-info col-6 col-sm-9'>
